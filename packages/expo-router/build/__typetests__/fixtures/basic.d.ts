 ...
 */
export type Route<T> = T extends string ? T extends DynamicRouteTemplate ? never : StaticRoutes | RelativePathString | ExternalPathString | (T extends `${infer P}${SearchOrHash}` ? P extends DynamicRoutes<infer _> ? T : never : T extends DynamicRoutes<infer _> ? T : never) : never;
/*********
 * Href  *
 *********/
export type Href<T = string> = T extends object ? HrefObject<T> : Route<T>;
export type HrefObject<R extends object, P = R extends {
    pathname: string;
} ? R['pathname'] : null> = P extends DynamicRouteTemplate ? {
    pathname: P;
    params: InputRouteParams<P>;
} : P extends Route<P> ? {
...
export declare const Redirect: <T>(props: React.PropsWithChildren<{
    href: Href<T>;
}>) => ReactNode;
/**
 * Hooks
 ...
 */
export declare function useLocalSearchParams<TParams extends AllRoutes | UnknownOutputParams = UnknownOutputParams>(): TParams extends AllRoutes ? SearchParams<TParams> : TParams;
/**
